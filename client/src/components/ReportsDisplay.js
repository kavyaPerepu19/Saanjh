...
const ReportsDisplay = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`https://saanjh-nine.vercel.app/api/reportData/${reportId}`);
      if (!response.ok) {
  ...
  const handleDiagnose = async () => {
    try {
      setLoading(true);
      const response = await axios.post('https://saanjh-nine.vercel.app/api/diagnose', { userId: reportData.userId, reportId });
  ...
  const fetchPreviousDiagnoses = async () => {
    try {
      setLoading(true);
      const response = await axios.post('https://saanjh-nine.vercel.app/api/previous-diagnoses', { reportIds: [reportId] });
          ...
          <div className="space-y-6">
            ...
            <div className="flex justify-center space-x-4">
              <button
                ...
                disabled={loading}
              >
                {loading ? <LoadingSpinner /> : 'Diagnose'}
              </button>
              <button
                ...
                disabled={loading}
              >
                {loading ? <LoadingSpinner /> : 'Show Previous Diagnoses'}
              </button>
            </div>
